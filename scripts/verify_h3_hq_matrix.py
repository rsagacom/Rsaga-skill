#!/usr/bin/env python3
"""Verify every generated H3 matrix video and emit a review manifest."""
...
RUN = ROOT / "test-runs/2026-08-13-h3-hq-matrix"
OUTPUTS = RUN / "outputs"
FRAMES = RUN / "frames"
...
def main() -> int:
    ...
    for item in manifest:
        ...
        print(f"{item['id']}\t{item.get('elapsed_seconds')}s\t{media.get('video_width')}x{media.get('video_height')}\t{verification.get('status')}\t{item.get('output','')}")
